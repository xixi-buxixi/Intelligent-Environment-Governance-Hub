...
TOKEN=$(curl -s -X POST http://localhost:8083/environment/api/auth/login \
  ...
  -d '{"username":"admin","password":"your_admin_password"}' | python3 -c "import sys,json;print(json.load(sys.stdin)['data']['token'])")
...
if d.get('code')==200:
    ...
    if exp:
        print('--- explanation (first 1000 chars) ---')
        print(exp[:1000])
    else:
    ...
    if sugg:
        for s in sugg[:3]:
            print(f'  - {s}')
    print('publicAdvice:', (data.get('publicAdvice','') or '')[:200])
else:
